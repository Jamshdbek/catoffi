 ...
 * Fullscreen notification window shown when a timer ends.
 * Loaded as a separate Electron window via main.cjs.
 * Background is fully transparent — only the centered content is visible.
 */
export default function NotificationScreen({ name, duration }) {
  const [visible, setVisible] = useState(false)
  ...
  return (
    <motion.div
      ...
      style={{
        width: '100vw',
        height: '100vh',
        // Background fully transparent — desktop behind shows through
        background: 'transparent',
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
      ...
      // Click outside the central card closes the notification
      onClick={handleClose}
    >
      {/* Animated expanding rings around the card (still transparent bg) */}
      {[0, 1, 2].map((i) => (
        <motion.div
          key={i}
      ...
      {/* Central round card — frosted glass over transparent bg */}
      <motion.div
        ...
        style={{
          ...
          height: 480,
          borderRadius: '50%',
          // Frosted glass: semi-transparent dark + heavy blur of what's behind
          background: 'rgba(15, 15, 15, 0.72)',
          backdropFilter: 'blur(40px) saturate(180%)',
          WebkitBackdropFilter: 'blur(40px) saturate(180%)',
          border: '1px solid rgba(255, 255, 255, 0.15)',
          boxShadow:
            '0 30px 80px rgba(0, 0, 0, 0.55), 0 0 0 1px rgba(255, 255, 255, 0.04) inset, 0 0 100px rgba(255, 255, 255, 0.06) inset',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
      ...
      >
        ...
        >
          {duration > 0
            ? `${fmtShort(duration)} session complete`
            : 'Session complete'}
        </motion.p>

        {/* Action buttons — compact for round card */}
        <motion.div
          initial={{ y: 16, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.7, duration: 0.5 }}
      ...
      {/* Auto-close hint below the card */}
      <motion.p
        initial={{ opacity: 0 }}
      ...
      >
        Click anywhere to dismiss · auto-closes in 30s
      </motion.p>
    </motion.div>
  )
}